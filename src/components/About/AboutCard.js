import React from "react";
import Card from "react-bootstrap/Card";
import { ImPointRight } from "react-icons/im";

function AboutCard() {
  return (
    <Card className="quote-card-view">
      <Card.Body>
        <blockquote className="blockquote mb-0">
          <p style={{ textAlign: "justify" }}>
            Hi Everyone, I am <span className="purple">Brosna REM </span>
            from <span className="purple"> Phnom Penh, Cambodia.</span>
            <br />
            I am currently employed as a software engineer at Branch of GUUD (International) Pte Ltd.
            <br />
            Experienced Software Engineer with Java background of hands-on experience in architecting, supporting, automating, and optimizing mission-critical deployments in AWS, leveraging Infrastructure as code, CI/CD, and Software processes.
            <br />
            <br />
            Apart from coding, some other activities that I love to do!
          </p>
          <ul>
            <li className="about-activity">
              <ImPointRight /> Digital Creator
            </li>
            <li className="about-activity">
              <ImPointRight /> Writing Code
            </li>
            <li className="about-activity">
              <ImPointRight /> Travelling
            </li>
          </ul>

          <p style={{ color: "rgb(155 126 172)" }}>
            "Strive to build things that make a difference!"{" "}
          </p>
          <footer className="blockquote-footer">Brosna</footer>
        </blockquote>
      </Card.Body>
    </Card>
  );
}

export default AboutCard;
